...
class FrameworkException(Exception):
  """Root exception class for framework."""
...
class Options(dict):
  ...
  def __keytransform__(self, key: str) -> str:
    """Return the uppercase form of a key."""
    return key.upper()

  def _boolify(self, value: str) -> bool:
    """Designed to throw an exception if value is not a string representation of a boolean."""
    return {'true': True, 'false': False}[value.lower()]

  def _autoconvert(self, value: Union[None, bool, str, int, float]) -> Union[None, bool, str, int, float]:
  ...
  def init_option(self, name: str, value=None, required: bool = False, description: str = '') -> None:
    """Create a new option."""
    name = self.__keytransform__(name)
    self[name] = value
  ...
  def serialize(self):
    """Create a serialized form of all options."""
    options = []
    for key in self:
      option = {}
      option['name'] = key
...
class Framework(cmd.Cmd):
  ...
  def default(self, line: str) -> None:
    """Respond with a default error."""
    self.error(f"Invalid command: {line}")

  def empty_line(self) -> int:
    """Disables running of last command when no command is given."""
    return 0

  def pre_cmd(self, line: str) -> str:
    """Run a few operations prior to processing the command."""
    if Framework._load:
      print('\r', end='')
  ...
  def print_topics(self, header, cmds, cmdlen, maxcol):
    ...
    if cmds:
      ...
      for cmd in cmds:
        self.stdout.write(
            f"{cmd.ljust(15)} {getattr(self, 'do_' + cmd).__doc__}{os.linesep}")
  ...
  def table(self, data, header=[], title=''):
    ...
    for i in range(0, cols):
      lens.append(len(max([self.to_unicode_str(x[i]) if x[i]
                           != None else '' for x in tdata], key=len)))
    ...
    if len(tdata) > 0:
      ...
      if header:
        ...
        print(separator)
      for rdata in tdata:
        data_sub = tuple([self.to_unicode_str(rdata[i]).ljust(
            lens[i]) if rdata[i] != None else ''.ljust(lens[i]) for i in range(0, cols)])
        print(data_str % data_sub)
  ...
  def insert_vulnerabilities(self, host=None, reference=None, example=None, publish_date=None, category=None, status=None, notes=None, mute=False):
    ...
    data = dict(
        ...
        reference=reference,
        example=example,
        publish_date=publish_date.strftime(
            self.time_format) if publish_date else None,
        category=category,
        status=status,
        notes=notes
    )
  ...
  def insert_ports(self, ip_address=None, host=None, port=None, protocol=None, banner=None, notes=None, mute=False):
    ...
    rowcount = self.insert('ports', data.copy(),
  ...
  def insert_contacts(self, first_name=None, middle_name=None, last_name=None, email=None, title=None, region=None, country=None, phone=None, notes=None, mute=False):
    ...
    rowcount = self.insert('contacts', data.copy(
  ...
  def insert_credentials(self, username=None, password=None, _hash=None, _type=None, leak=None, notes=None, mute=False):
    ...
    if username is not None and '@' in username:
      self.insert_contacts(first_name=None, last_name=None,
                           title=None, email=username)
  ...
  def insert(self, table, data, unique_columns=[]):
    ...
    data['module'] = 'user_defined' if '_do_db_insert' in [x[3]
                                                           for x in inspect.stack()] else self._modulename.split('/')[-1]
    ...
    unique_columns = [
        x for x in unique_columns if x in columns and x != 'module']
    ...
    unique_columns_str = ' and '.join(
        [f"`{column}`=?" for column in unique_columns])
    ...
    values = tuple([data[column] for column in columns] +
                   [data[column] for column in unique_columns])
  ...
  def register_option(self, name, value, required, description):
    self.options.init_option(name=name, value=value,
      ...
      if not type(self.options[option]) in [bool, int]:
        if self.options.required[option] is True and not self.options[option]:
          raise FrameworkException(
              f"Value required for the '{option}' option.")
    return
  ...
  def _list_options(self, options=None):
    ...
    if options:
      ...
      print(pattern % (self.ruler * key_len, (self.ruler *
                                              13).ljust(val_len), self.ruler * 8, self.ruler * 11))
      for key in sorted(options):
        ...
        print(pattern % (key.ljust(key_len), self.to_unicode_str(
            value).ljust(val_len), self.to_unicode_str(reqd).ljust(8), desc))
      print('')
    else:
      print('')
  ...
  def request(self, method, url, **kwargs):
    ...
    kwargs['timeout'] = kwargs.get(
        'timeout') or self._global_options['timeout']
    # process headers
  ...
  def _do_db_notes(self, params):
    ...
    if table in self.get_tables():
      ...
      for rowid in rowids:
        count += self.query(
            f"UPDATE `{table}` SET notes=? WHERE ROWID IS ?", (note, rowid))
  ...
  def _do_db_insert(self, params):
    ...
    if table in self.get_tables():
      ...
      def sanitize_column(x): return '_' + x if x in ['hash', 'type'] else x
      record = {}
  ...
  def _do_db_delete(self, params):
    ...
    if table in self.get_tables():
      ...
      for rowid in rowids:
        count += self.query(
            f"DELETE FROM `{table}` WHERE ROWID IS ?", (rowid,))
      self.output(f"{count} rows affected.")
    else:
      self.output('Invalid table name.')
  ...
  def _do_script_stop(self, params):
    ...
    if Framework._record:
      self.output(
          f"Recording stopped. Commands saved to '{Framework._record}'.")
  ...
  def _do_spool_stop(self, params):
    ...
    if Framework._spool:
      self.output(
          f"Spooling stopped. Output saved to '{Framework._spool.name}'.")
  ...
  def help_options(self):
    ...
    print(
        f"{os.linesep}Usage: options <{'|'.join(self._parse_subcommands('options'))}> [...]{os.linesep}")
  ...
  def help_keys(self):
    ...
    print(
        f"{os.linesep}Usage: keys <{'|'.join(self._parse_subcommands('keys'))}> [...]{os.linesep}")
  ...
  def help_modules(self):
    ...
    print(
        f"{os.linesep}Usage: modules <{'|'.join(self._parse_subcommands('modules'))}> [...]{os.linesep}")
  ...
  def help_db(self):
    ...
    print(
        f"{os.linesep}Usage: db <{'|'.join(self._parse_subcommands('db'))}> [...]{os.linesep}")

  def _help_db_notes(self):
    print(getattr(self, '_do_db_notes').__doc__)
    print(
        f"{os.linesep}Usage: db note <table> [<rowid(s)> <note>]{os.linesep}")
    print(
        f"rowid(s) => ',' delimited values or '-' delimited ranges representing rowids{os.linesep}")
  ...
  def _help_db_insert(self):
    ...
    print(
        f"values => '~' delimited string representing column values (exclude rowid, module){os.linesep}")
  ...
  def help_script(self):
    ...
    print(
        f"{os.linesep}Usage: script <{'|'.join(self._parse_subcommands('script'))}> [...]{os.linesep}")
  ...
  def help_spool(self):
    ...
    print(
        f"{os.linesep}Usage: spool <{'|'.join(self._parse_subcommands('spool'))}> [...]{os.linesep}")
